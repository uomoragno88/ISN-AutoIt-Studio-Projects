...
ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $a_distinta_Amazon = ' & $a_distinta_Amazon[0] & @crlf & '>Error code: ' & @error & @crlf) ;### Debug Console
Select
...
EndSelect
Until $i_offset_ok
ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $i_offset_ok = ' & $i_offset_ok & @crlf & '>Error code: ' & @error & @crlf) ;### Debug Console
...
;~ 		MsgBox($MB_OK, "Errore", "Non ci sono ordini da elaborare!")
		$sMsg = "Non ci sono ordini da elaborare! Elaborazione abortita"
		$iRetValue = _ExtMsgBox($EMB_ICONSTOP, "OK", "Errore", $sMsg)
;~ 		MsgBox
		Exit
	EndIf
...
;~ 			MsgBox($MB_OK, "Prosegui", "Elaborazione ordine terminata. Clicca OK per proseguire")
			$sMsg = "Elaborazione ordine terminata. Clicca OK per proseguire"
			$iRetValue = _ExtMsgBox($EMB_ICONINFO, "OK", "Prosegui", $sMsg)
			GUICtrlSetState($hProsegui, $GUI_FOCUS)
		Else

...
;if use dbug disable "MustDeclareVars"
Opt("MustDeclareVars", 0) ;0=no, 1=require pre-declare

Global $aParams[1][4]
; column:
; [1] type [2] name [3] method [4] structure name
Global $aStructs[1][3]
; column:
; [1] structure name [2] type [3] name 
Local $nMsg
Local $sDllCallOut
Local $nExit
Local $ireturncode
...
;~ 			If GUICtrlRead($ComboParamByref) = "ByRef" And StringLeft(GUICtrlRead($InputParamValue), 1) <> "$" Then
			...
			ReDim $aParams[UBound($aParams) + 1][4] ; Add a "row" to the array
...
Func GenerateCode($sOutputType = "")
	...
	If UBound($aStructs) > 1 Then
		$sDllCallOut &= "Local $tag" & $aStructs[1][0] & " = 'struct;"

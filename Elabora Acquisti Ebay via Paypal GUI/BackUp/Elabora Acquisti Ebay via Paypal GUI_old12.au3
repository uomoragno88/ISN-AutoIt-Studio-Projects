...
;~ 					GUICtrlSetData($h_PREZZO_MINIMO_VENDITA_DATA, $ItemArray[$i_row][3])
...
;~ 								test per numero pezzi inserito e numerico
								$s_query = "SELECT * FROM ACQUISTI"
								RegistraAcquisto($s_dbname, $s_query, $o_Con)
								$s_query = "SELECT * FROM MOVIMENTI"
								RegistraMovimento($s_dbname, $s_query, $o_Con)
						EndSwitch
					WEnd
					; Clear up

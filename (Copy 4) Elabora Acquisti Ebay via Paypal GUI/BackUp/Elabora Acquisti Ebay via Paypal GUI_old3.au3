...
Func Tratta_Paypal($GUI_Form, $hArea_Comunicazioni)
	...
	$File = FileOpenDialog("Select a TAB File", @DesktopDir, "TAB Files (*.tab)")
	$Input = FileRead($File)
	$Delimiter = @TAB
...
;~ ======================
	...
	Local $i_almeno1, $s_work_tipo, $s_work_tariffe
	$i_almeno1 = 0
	
	For $i_row = 3 to $i_max_row
		$s_works_tipo = $ItemArray[$i_row][3]
		$s_works_tariffe = $ItemArray[$i_row][34]
		$s_works_tariffe = StringReplace($s_works_tariffe, ",", ".")
		Select
			Case $s_works_tipo = "Pagamento asta eBay"
				If $s_works_tariffe = 0 Then
					$i_almeno1 = 1
					ExitLoop
				EndIf
		EndSelect
	...
	For $i_row = 3 to $i_max_row
		...
		$s_works_tariffe = StringReplace($s_works_tariffe, ",", ".")
		Select
			Case $s_works_tipo = "Conversione di valuta generica"
				$s_TAB_ID_transazione_di_riferimento = $ItemArray[$i_row][1]
				$s_TAB_Data = $ItemArray[$i_row][2]
				$s_TAB_Numero_oggetto = $ItemArray[$i_row][5]
				$s_TAB_Campo_opzionale = $ItemArray[$i_row][22]
				$s_TAB_Valuta = $ItemArray[$i_row][33]
				$s_TAB_Importo_netto = $ItemArray[$i_row][35]
				Select
			...
			Case $s_works_tipo = "Pagamento asta eBay"
				Select
					Case $s_works_tariffe = 0
					$s_TAB_ID_transazione = $ItemArray[$i_row][0]
					If $s_TAB_ID_transazione = $s_TAB_ID_transazione_di_riferimento Then
					Else
					;;;;;;;;;;errore grave
					EndIf
					$s_TAB_Nome = $ItemArray[$i_row][8]
					$s_TAB_Email = $ItemArray[$i_row][9]
					Case $s_works_tariffe <> 0
						ExitLoop
				EndSelect
		EndSelect

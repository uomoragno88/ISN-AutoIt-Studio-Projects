...
;~ ======================
	...
	; tratto gli ACQUISTI
	Global $s_TAB_ID_transazione
	Global $s_TAB_ID_transazione_di_riferimento
	Global $s_TAB_Data
	Global $s_TAB_Nome
	Global $s_TAB_Email
	Global $s_TAB_Numero_oggetto
	Global $s_TAB_Campo_opzionale
	Global $s_TAB_Valuta
	Global $s_TAB_Importo_netto
		...
		Select
			...
			Case $s_works_tipo = "Pagamento con procedura rapida inviato"
				If $s_work_cod_oggetto <> "" Then
					...
					$s_work_ID_ACQ = IncrementaID_ACQ($s_ACQUISTI_ID_ACQ, $s_ACQUISTI_DATA_ACQUISTO)
					; recupero ID fornitore
					$s_TAB_Nome = "'" & $s_TAB_Nome & "'"
					$s_query = "SELECT * FROM FORNITORI WHERE COD_FORNITORE =" & $s_TAB_Nome & ""
					$s_ACQUISTI_FORNITORE = RicercaID_Fornitore($s_dbname, $s_query, $o_Con)
					; recupero descizione valuta
					$s_TAB_Valuta = "'" & $s_TAB_Valuta & "'"
					$s_query = "SELECT * FROM VALUTE WHERE ID_VALUTA =" & $s_TAB_Valuta & ""
					$s_ACQUISTI_VALUTA = RicercaDescr_Valuta($s_dbname, $s_query, $o_Con)

					; imposto il form 
					GUICtrlSetData($h_ID_ACQ_DATA, $s_work_ID_ACQ)
					GUICtrlSetData($hANNO_CONTABILE_DATA, $s_ACQUISTI_ANNO_CONTABILE)
					GUICtrlSetData($hDOCUMENTO_DI_CARICO_DATA, $s_ACQUISTI_DOC_DI_CARICO_NUMERO)
